...
class FHSS {
 ...
 private:
  ...
  bool bindButtonPressed() { return (*bind_in_port_ & bind_bitmask_) == 0; }
 ...
 public:
  ...
  // Execute callback, return next callback interval in us. If return is 0, then
  // no callback
  uint16_t callback() {
    // Start binding
    ...
    if (is_binding_) {
      ...
      return kCallbackInterval;  // For binding packets, use normal callback
                                 // interval
      ...
      // In KT531p implementation, all packets are prepared and loaded to the RF
      // chip But the TX Strobe command is issued only on every other packet.
      // The reason for this is unknown.
      // However, this means we can safety skip some packets.
      // This allows us to send to two receivers with one modem
      // Since it takes 3.1 ms to transmit a packet, and there's only 3.8ms
      // between hopping. If we skip half the packets, we have 7.6ms and can
      // alternate between two receivers.
      if (!(freq_idx_ & 0b1)) {
        sendNormalPacket();
      }
      // For normal packets, send every other packet, as official firmware does
      freq_idx_ =
          (freq_idx_ + 2) & 0x1F;  // Take lower 6 bits, equivalent to mod 32
    ...
    while (idx < num_ch) {
      ...
      if (idx != 0) {
        if (freq_table_[idx - 1] > next_ch) {
          if (freq_table_[idx - 1] - next_ch < 5) continue;
        } else if (next_ch - freq_table_[idx - 1] < 5)
          continue;
      }
      // Check that it's not duplicated and spread uniformly
      for (i = 0; i < idx; i++) {
        if (freq_table_[i] == next_ch) break;
        if (freq_table_[i] <= 26)
          count_2_26++;
