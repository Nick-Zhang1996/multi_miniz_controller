...
FHSS trans2(modem1, 15,0x4E5235, g_pwm_data+2, g_pwm_data+3); // Bind pin on D15 (A1)

A7105 modem2(3); // CSN on D3
FHSS trans3(modem2, 16,0x521234, g_pwm_data+4, g_pwm_data+5); // Bind pin on D16 (A2)
FHSS trans4(modem2, 17,0x376423, g_pwm_data+6, g_pwm_data+7); // Bind pin on D17 (A3)

A7105 modem3(4); // CSN on D4
FHSS trans5(modem3, 18,0x65AB28, g_pwm_data+8, g_pwm_data+9); // Bind pin on D18 (A4)
FHSS trans6(modem3, 19,0x72CD79, g_pwm_data+10, g_pwm_data+11); // Bind pin on D19 (A5)
...
uint16_t task_ts[kTaskSize] = {0, 400*2, 400*4, 1600+kTransMargin*2 , 1600+kTransMargin*2 + 400*2, 1600+kTransMargin*2+400*4};
// Callback for trans, note the spacing, trans1 and trans2 share a modem, 
// allow sufficient time for transmission 
FHSS* task_target[kTaskSize] = {&trans1, &trans3, &trans5, &trans2, &trans4, &trans6};
