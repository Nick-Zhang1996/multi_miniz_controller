...
static void __attribute__((unused)) KYOSHO_hype_send_packet() {
  if (IS_BIND_IN_PROGRESS) {
    if (packet_sent == 0) { // build the packet and send it
    ...
    // original TX is only refreshing the packet every 20ms and keep repeating
    // the same packet in between (STROBE_TX) build packet=6 channels with order
    // AETR
    ...
    packet_period =
        931; // packet period fluctuates a lot on the original TX from one
             // packet to the other but stable if looking over a period of 40ms
#if 0
			debug("ch=%02X P=",rf_ch_num);
			for(uint8_t i=0; i<6; i++)
				debug("%02X ", packet[i]);
...
void KYOSHO_init() {
  ...
  if (sub_protocol == KYOSHO_FHSS) {
    ...
    memcpy(hopping_frequency,
           "\x73\x12\x7D\x88\x63\x4A\x8D\x60\x57\x16\x5D\x8B\x25\x53\x6E\x3C"
           "\x41\x70\x20\x83\x2A\x19\x94\x2F\x91\x4C\x47\x36\x78\x10\x5A\x31",
           32);
  }
#endif
  if (sub_protocol == KYOSHO_HYPE) {
    ...
    memcpy(hopping_frequency,
           "\x27\x1B\x63\x75\x03\x39\x57\x69\x87\x0F\x7B\x3F\x33\x51\x6F", 15);
#endif
    if (IS_BIND_IN_PROGRESS)
