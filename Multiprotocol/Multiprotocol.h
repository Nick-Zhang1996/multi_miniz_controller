...
enum { PKT_PENDING = 0, PKT_ACKED, PKT_TIMEOUT };
...
#define MODELMODE_EEPROM_OFFSET                                                \
  30 // Autobind mode, 1 byte per model, end is 30+16=46
#define AFHDS2A_EEPROM_OFFSET                                                  \
  50                           // RX ID, 4 bytes per model id, end is 50+64=114
#define BUGS_EEPROM_OFFSET 114 // RX ID, 2 bytes per model id, end is 114+32=146
#define BUGSMINI_EEPROM_OFFSET                                                 \
  146 // RX ID, 2 bytes per model id, end is 146+32=178
#define FRSKY_RX_EEPROM_OFFSET                                                 \
  178 // (1) format + (3) TX ID + (1) freq_tune + (47) channels, 52 bytes, end
      // is 178+52=230
#define AFHDS2A_RX_EEPROM_OFFSET                                               \
  230 // (4) TX ID + (16) channels, 20 bytes, end is 230+20=250
#define AFHDS2A_EEPROM_OFFSET2                                                 \
  250 // RX ID, 4 bytes per model id, end is 250+192=442
#define HOTT_EEPROM_OFFSET                                                     \
  442 // RX ID, 5 bytes per model id, end is 320+442=762
#define BAYANG_RX_EEPROM_OFFSET                                                \
  762 // (5) TX ID + (4) channels, 9 bytes, end is 771
#define FRSKYD_CLONE_EEPROM_OFFSET                                             \
  771 // (1) format + (3) TX ID + (47) channels, 51 bytes, end is 822
#define FRSKYX_CLONE_EEPROM_OFFSET                                             \
  822 // (1) format + (3) TX ID + (47) channels, 51 bytes, end is 873
#define FRSKYX2_CLONE_EEPROM_OFFSET                                            \
  873                            // (1) format + (3) TX ID, 4 bytes, end is 877
#define DSM_RX_EEPROM_OFFSET 877 // (4) TX ID + format, 5 bytes, end is 882
#define MOULDKG_EEPROM_OFFSET                                                  \
  882 // RX ID, 3 bytes per model, end is 882+64*3=1074
#define DSM_CLONE_EEPROM_OFFSET 1074 // (4) TX ID, (1) Initialized, end is 1079
#define TRAXXAS_EEPROM_OFFSET                                                  \
  1079 // RX ID and SOP index, 3 bytes per model id, end is 1079+192=1271
#define XK2_EEPROM_OFFSET                                                      \
  1271 // RX ID checksum, 1 byte per model, end is 1271+64=1335
#define JIABAILE_EEPROM_OFFSET                                                 \
  1335 // RX ID, 3 bytes per model, end is 1335+64*3=1527
// #define CONFIG_EEPROM_OFFSET 	1527	// Current configuration of the
// multimodule
...
Serial: 100000 Baud 8e2      _ xxxx xxxx p --
  ...
  Stream[0]   = header
                                0x55	sub_protocol values are 0..31	Stream
contains channels 0x54	sub_protocol values are 32..63	Stream contains channels
                                0x57	sub_protocol values are 0..31	Stream
contains failsafe 0x56	sub_protocol values are 32..63	Stream contains failsafe
                                Note: V2 adds the 2 top bits to extend the
number of protocols to 256 in Stream[26] Stream[1]   =
sub_protocol|BindBit|RangeCheckBit|AutoBindBit; sub_protocol is 0..31 (bits
0..4) Reserved	0 Flysky		1 Hubsan		2 FrskyD
3 Hisky		4 V2x2		5 DSM			6 Devo		7 YD717
8 KN			9 SymaX		10 SLT			11 CX10
12 CG023		13 Bayang		14 FrskyX		15 ESky
16 MT99XX		17 MJXQ		18 SHENQI		19 FY326
20 Futaba		21 J6PRO		22 FQ777		23 ASSAN
24 FrskyV		25 HONTAI		26 OpenLRS		27
                                AFHDS2A		28
                                Q2X2		29
                                WK2x01		30
                                Q303		31
  ...
  Stream[2]   = RxNum | Power | Type;
   ...
   Type is 0..7 <<4     (bit 4..6)
                ...
                sub_protocol==CX10
                        CX10_GREEN	0
                        CX10_BLUE	1	// also compatible with CX10-A,
CX12 DM007		2
                        ---			3
                        JC3015_1	4
                        JC3015_2	5
  ...
  Stream[4] to [25] = Channels or failsafe depending on Steam[0]
   ...
   Failsafe values have exactly the same range/values than normal channels
except the extremes where 0=no pulse, 2047=hold. If failsafe is not set or RX
then failsafe packets should not be sent. Stream[26]   = sub_protocol bits 6 &
7|RxNum bits 4 & 5|Telemetry_Invert 3|Future_Use 2|Disable_Telemetry
1|Disable_CH_Mapping 0 sub_protocol is 0..255 (bits 0..5 + bits 6..7) RxNum
value is 0..63 (bits 0..3 + bits 4..5) Telemetry_Invert		=> 0x08
0=normal, 1=invert Future_Use			=> 0x04	0=      , 1=
  ...
  Stream[27.. 35] = between 0 and 9 bytes for additional protocol data
    Protocol specific use:
      FrSkyX and FrSkyX2: Stream[27] during bind Telem on=0x00,off=0x01 |
CH1-8=0x00,CH9-16=0x02 FrSkyX and FrSkyX2: Stream[27..34] during normal
operation unstuffed SPort data to be sent HoTT: Stream[27] 1 byte for telemetry
type DSM: Stream[27..33] Forward Programming
*/
/*
  Multiprotocol telemetry/command definition for OpenTX and erskyTX
  Based on #define MULTI_TELEMETRY enables OpenTX and erskyTX to get the
  multimodule status and select the correct telemetry type automatically.
  ...
  Format: header (4 byte) + data (variable)
   ...
   The first byte is deliberatly chosen to be different from other telemetry
  protocols (e.g. 0xAA for DSM/Multi, 0xAA for FlySky and 0x7e for Frsky) to
  allow a TX to detect the telemetry format of older versions
  ...
  Type = 0x01 Multimodule Status:
   ...
   [7] revision
   [8] patchlevel
     version of multi code, should be displayed as
  major.minor.revision.patchlevel [9] channel order: CH4|CH3|CH2|CH1 with CHx
  value A=0,E=1,T=2,R=3 [10] Next valid protocol number, can be used to skip
  invalid protocols [11] Prev valid protocol number, can be used to skip invalid
  protocols [12..18] Protocol name [7], not null terminated if prototcol len ==
  7 [19>>4] Option text to be displayed: OPTION_NONE		0	Hidden
  field OPTION_OPTION	1	"Option:"
  value=-128..0(default)..127 OPTION_RFTUNE	2	"RF freq tune:"
  value=-128..0(default)..127 OPTION_VIDFREQ	3	"Video freq:"
  value=-128..0(default)..127 OPTION_FIXEDID	4	"ID type:"
  value="Auto":0(default), "Fixed":1 OPTION_TELEM	5	"Telem:"
  value="Off":0(default), "On":1, "Off+Aux":2, "On+Aux":3 OPTION_SRVFREQ
  6	"Servo freq(Hz):"	value="50":0(default).."400":70 =>
  display=50+5*option with option=0..70 OPTION_MAXTHR	7	"Max throw:"
  value="Disabled":0, "Enabled":1 OPTION_RFCHAN	8	"Select RF chan:"
  value=-128..0(default)..127 OPTION_RFPOWER	9	"RF power:"
  "1.6mW":0(default),"2.0mW":1,"2.5mW":2,"3.2mW":3,"4.0mW":4,"5.0mW":5,"6.3mW":6,"7.9mW":7,"10mW\0":8,"13mW\0":9,"16mW\0":10,"20mW\0":11,"25mW\0":12,"32mW\0":13,"40mW\0":14,"50mW\0":15
                        OPTION_WBUS		10	"Output:"
  "WBUS":0(default),"PPM":1 [19&0x0F] Number of sub protocols [20..27] Sub
  protocol name [8], not null terminated if sub prototcol len == 8 If the
  current protocol is invalid [12..27] are all 0x00.

   more information can be added by specifing a longer length of the type, the
  TX will just ignore these bytes
  ...
  Type 0x08 Input synchronisation
    ...
    data[2-3]     Time (??s) between last serial servo input received and servo
  input needed (lateness), TX should adjust its sending time to minimise this
  value. data[4]		  Interval of this message in ms data[5]
  Input delay target in 10??s Note that there are protocols (AFHDS2A) that have
  a refresh rate that is smaller than the maximum achievable refresh rate via
  the serial protocol, in this case, the TX should double the rate and also
  subract this refresh rate from the input lag if the input lag is more than the
  desired refresh rate. The remote should try to get to zero of
  (inputdelay+target*10).
  ...
  Type 0x11 Protocol list export via telemetry. Used by the protocol
  PROTO_PROTOLIST=0, the list entry is given by the Option field. length:
  variable data[0]     = protocol number, 0xFF is an invalid list entry (Option
  value too large), Option == 0xFF -> number of protocols in the list data[1..n]
  = protocol name null terminated data[n+1]   = flags flags>>4 Option text
  number to be displayed (check multi status for description) flags&0x01
  failsafe supported flags&0x02 Channel Map Disabled supported data[n+2]   =
  number of sub protocols data[n+3]   = sub protocols text length, only sent if
  nbr_sub != 0 data[n+4..] = sub protocol names, only sent if nbr_sub != 0
